...
- **Holtzman et al. (2020)** "The Curious Case of Neural Text Degeneration" — 参考了 Section 3-4（文本退化分析、Nucleus Sampling 方法）
- **Fan et al. (2018)** "Hierarchical Neural Story Generation" — 参考了 Top-k Sampling 方法

### 教材
- **D2L** Section 15.8-15.10 (BERT) — 参考了ELMo/GPT/BERT对比图和教学组织方式
- **D2L** Section 10.8 (Beam Search) — 参考了束搜索的图解和教学组织
- **SLP3** Chapter 10-11 — 参考了预训练范式的讲解框架、解码策略（Ch 10.3 Beam Search, Ch 10.7 Sampling Methods）

### 课程
- **Stanford CS224N** Lecture 9-11 (2025) "Pretraining & NLG" — 参考了GPT架构讲解、预训练范式对比、解码策略
- **CMU 11-711** ANLP (2024) — 参考了autoregressive LM的理论分析和生成方法
:::
...
上一节我们看到，GPT的因果语言模型在每个时间步产生一个概率分布 $P(x_t \mid x_1, \ldots, x_{t-1})$——对词汇表中每个token分配一个概率。预训练的目标是让这个分布尽可能准确地建模真实的语言。但训练完成之后，一个实际的问题随之而来：**拿到这个概率分布后，我们应该如何从中选择下一个token来生成文本？**

这个看似简单的问题隐藏着巨大的设计空间。不同的选择策略——称为**解码策略（decoding strategy）**——会产生截然不同的文本质量和风格。理解解码策略不仅对使用GPT至关重要，也是理解后续章节中In-Context Learning（第20章）和Chain-of-Thought推理（第21章）的前置知识。

#### 问题定义：自回归生成是一系列选择

自回归文本生成的过程可以形式化为：给定已生成的前缀 $x_{1:t-1}$，模型计算下一个token的概率分布 $P(x_t \mid x_{1:t-1})$，然后**从这个分布中选择一个token**，将其追加到前缀中，重复这个过程直到生成结束标记或达到最大长度。

$$
x_t \sim \text{Decode}\big(P(\cdot \mid x_1, \ldots, x_{t-1})\big), \quad t = 1, 2, \ldots, T
$$

关键在于 $\text{Decode}(\cdot)$ 的设计——从概率分布到具体token的映射有无穷多种方式，每种方式都带来不同的权衡。

#### 贪心解码：最简单但最无聊的选择

最直接的方法是**贪心解码（Greedy Decoding）**：每一步都选择概率最高的token。

$$
x_t = \arg\max_{w \in V} P(w \mid x_1, \ldots, x_{t-1})
$$

贪心解码是确定性的——相同的输入总是产生相同的输出，每一步的选择在局部看来都是"最优的"。但它有一个致命的缺陷：**局部最优不等于全局最优**。想象模型在生成故事时，每一步都选了概率最高的词——"The"、"man"、"was"——每步都合理，但整体陷入一个平淡、重复的叙事模式。

更严重的是，贪心解码生成的文本往往出现**退化现象（degeneration）**——反复生成相同的短语或陷入循环。Holtzman et al. (2020) 的研究揭示了一个关键发现：贪心解码生成的文本在概率分布上与真实人类文本有显著差异。模型生成的文本倾向于选择高概率的"安全"词汇，而人类写作则经常选择中等概率甚至低概率的词汇——正是这种"意外性"让文本变得有趣和自然。

#### 束搜索：保留多个候选

**束搜索（Beam Search）**是对贪心解码的直接改进。它的核心思想很直接：既然贪心解码的问题是只保留一个候选，那我们就保留多个。具体来说，束搜索在每一步保留概率最高的 $B$ 个候选序列（$B$ 称为**束宽**，beam width）。算法过程如下：

1. **初始化**：从起始token开始，计算词表中每个token的概率，选取概率最高的 $B$ 个作为初始候选。
2. **扩展**：对每个现有候选序列，计算所有可能的续写token的概率，得到 $B \times |V|$ 个候选。
3. **剪枝**：从这 $B \times |V|$ 个候选中，按**累积对数概率**选取最高的 $B$ 个保留。
4. **终止**：当候选序列生成结束符（`<EOS>`）或达到最大长度时停止，输出累积概率最高的完整序列。

用对数概率是因为连乘容易下溢。一个长度为 $T$ 的序列的得分为：

$$
\text{score}(x_{1:T}) = \sum_{t=1}^{T} \log P(x_t \mid x_{<t})
$$

注意束搜索的两个边界情况：$B = 1$ 时退化为贪心解码，$B = |V|^T$ 时等价于穷举搜索。实践中 $B$ 通常取 $4 \sim 10$，在计算代价和搜索质量之间取得平衡。

**数值示例：束宽 $B=2$ 的两步展开。** 假设模型要续写"The cat"，词表简化为 {sat, is, ran, the}。

**第1步**——计算 $P(x_3 \mid \text{The cat})$：
...
$B=2$，保留 Top-2 候选：**"The cat sat"**（得分 −0.92）和 **"The cat is"**（得分 −1.05）。

**第2步**——分别对两个候选计算续写概率：

| 候选序列 | 续写token | $P(x_4 \mid \cdot)$ | 累积 $\log P$ |
|:---------|:----------|:---------------------|:--------------|
| The cat sat | on    | 0.50                 | −0.92 + (−0.69) = **−1.61** |
| The cat sat | down  | 0.30                 | −0.92 + (−1.20) = −2.12 |
| The cat sat | there | 0.10                 | −0.92 + (−2.30) = −3.22 |
| The cat sat | the   | 0.10                 | −0.92 + (−2.30) = −3.22 |
| The cat is  | on    | 0.30                 | −1.05 + (−1.20) = **−2.25** |
| The cat is  | very  | 0.25                 | −1.05 + (−1.39) = −2.44 |
| The cat is  | a     | 0.25                 | −1.05 + (−1.39) = −2.44 |
| The cat is  | the   | 0.20                 | −1.05 + (−1.61) = −2.66 |

从全部 $2 \times 4 = 8$ 个候选中选 Top-2：**"The cat sat on"**（−1.61）和 **"The cat is on"**（−2.25）。注意贪心解码只会走"The cat sat → on"这一条路径，而束搜索同时追踪了"The cat is"分支，保留了找到全局更优序列的可能性。

**长度偏好问题与长度惩罚。** 上面的得分公式 $\sum \log P$ 有一个隐含的偏差：对数概率都是负数，序列越长，累积得分越低。这意味着束搜索会系统性地偏好短序列——一个只有3个词的翻译，可能仅仅因为"短"就得分高于一个更完整但更长的翻译。

为此，实践中通常加入**长度惩罚（length penalty）**来归一化：

$$
\text{score}_{\text{lp}}(x_{1:T}) = \frac{1}{T^\alpha} \sum_{t=1}^{T} \log P(x_t \mid x_{<t})
$$

其中 $\alpha \in [0, 1]$ 控制惩罚强度。$\alpha = 0$ 时不做归一化（偏好短序列），$\alpha = 1$ 时完全按平均对数概率排序，实践中 $\alpha = 0.6 \sim 0.7$ 效果较好。Google的GNMT系统（Wu et al., 2016）使用了一个略有不同的形式 $\text{lp}(T) = \frac{(5 + T)^\alpha}{(5 + 1)^\alpha}$，加入常数5是为了避免对极短序列过度补偿。

**束搜索的黄金时代与局限。** 在机器翻译的时代（2014–2019），束搜索几乎是标配。翻译任务有较明确的"正确答案"，源语言句子强烈地约束了目标语言的分布，"最高概率的翻译"通常就是"最好的翻译"。束搜索能有效地在这个受约束的搜索空间中找到高概率的结果。

然而，在开放式文本生成的场景下——故事续写、对话、创意写作——束搜索的表现令人失望。它生成的文本虽然语法正确，但往往枯燥、重复、缺乏创造性——就像一个只会说"正确但无聊的话"的人。根本原因在于**自然语言的概率分布与"最优序列"之间的关系**。在开放式生成中，下一个词的分布是扁平的——有很多合理的续写方式，而"概率最高的续写"往往是最平庸的那一种。Holtzman et al. (2020) 用数据量化了这一点：在人类写作的文本中，下一个词的概率排名中位数远不是第一名——人类写作的魅力恰恰在于偶尔选择不那么"显而易见"的词。束搜索永远无法捕捉这种"有意义的意外性"，因为它的目标函数就是最大化概率。

#### Temperature Scaling：控制分布的"尖锐度"

从贪心和束搜索的失败中我们意识到，好的文本生成需要一定的**随机性**。最基本的做法是直接按模型输出的概率分布随机采样，但原始分布可能太尖锐或太平坦。**温度缩放（Temperature Scaling）**提供了一个优雅的控制手段——在softmax之前用温度参数 $T$ 除以logits：

$$
P_T(x_t = w_i) = \frac{\exp(z_i / T)}{\sum_j \exp(z_j / T)}
$$

其中 $z_i$ 是模型输出的原始logit值。温度的效果非常直观。当 $T < 1$ 时，分布变得更"尖锐"——高概率token的概率进一步提高，低概率token被压制，极端情况 $T \to 0$ 退化为贪心解码。当 $T > 1$ 时，分布变得更"平坦"——各token的概率差距缩小，低概率token也有更多机会被选中，极端情况 $T \to \infty$ 退化为均匀随机采样。当 $T = 1$ 时就是模型原始分布。

温度的数学本质可以用一个简洁的公式理解。如果原始概率为 $P_i$，那么温度缩放后的概率正比于 $P_i^{1/T}$。低温度相当于对概率取更高次幂——放大了概率之间的差距；高温度相当于取更低次幂——缩小了差距。

#### Top-k 采样：截断长尾

温度缩放调节了分布的形状，但即使温度较低，概率极小的token仍然有被采样到的可能——这可能导致生成无意义的词。**Top-k采样**（Fan et al., 2018）提供了一个更直接的方案：只保留概率最高的 $k$ 个token，将其余token的概率设为零，然后在这 $k$ 个token中按重新归一化的概率采样。

Top-k采样有效地过滤了长尾中的低质量候选，同时在高概率token之间保留了随机性。但它有一个显著的缺陷：**$k$ 是固定的**。然而不同的上下文需要不同程度的多样性。有些位置的下一个词几乎是确定的——"United States of"之后几乎一定是"America"，此时即使 $k = 2$ 都嫌多；而有些位置有很多合理的选择——"I like to eat"之后可以是各种食物，此时 $k = 40$ 可能不够。固定的 $k$ 无法适应这种变化。

#### Nucleus采样（Top-p）：自适应的多样性控制

Holtzman et al. (2020) 在论文"The Curious Case of Neural Text Degeneration"中提出了一个优雅的解决方案：**Nucleus采样（Top-p采样）**。核心思想是不固定候选token的数量，而是固定候选token的**累积概率**。

给定阈值 $p$（如 $p = 0.9$），Nucleus采样选择**最小的token集合** $V^{(p)}$，使得这些token的累积概率至少为 $p$：

$$
V^{(p)} = \arg\min_{V' \subseteq V} |V'| \quad \text{s.t.} \quad \sum_{w_i \in V'} P(w_i) \geq p
$$

然后从 $V^{(p)}$ 中按重新归一化的概率采样。

这个设计的精妙之处在于它是**自适应的**。当模型对下一个词非常确定时（概率分布集中在少数几个token上），$V^{(p)}$ 可能只包含2–3个token；当模型不确定时（概率分布分散），$V^{(p)}$ 可能包含几十甚至上百个token。这种动态调整恰好匹配了语言生成的需求——在该确定的地方确定，在该多样的地方多样。

Top-p采样因其简洁性和有效性迅速成为LLM时代的默认选择。OpenAI的API、Hugging Face的generate函数、以及几乎所有主流推理框架都将top-p作为主要的采样参数，通常与temperature组合使用。

#### 完整数值示例：不同解码策略的对比

让我们通过一个具体的数值例子来直观理解各种解码策略的差异。

**设定**：模型已经生成了 "The cat sat on the"，现在需要预测下一个词。模型输出的原始概率分布（$T=1.0$）如下：
...
**Step 1：Greedy Decoding** → 直接选概率最高的 **"mat"**（0.35）。无论运行多少次，结果都一样。

**Step 2：Temperature Scaling 的效果**

温度缩放后的概率正比于 $P_i^{1/T}$，归一化后：

| Token | $T=1.0$（原始） | $T=0.5$（更确定） | $T=1.5$（更随机） |
|-------|---------|---------|---------|
| mat | 0.35 | **0.54** | 0.27 |
| floor | 0.25 | 0.28 | 0.22 |
| bed | 0.15 | 0.10 | 0.16 |
| chair | 0.10 | 0.04 | 0.12 |
| rug | 0.08 | 0.03 | 0.10 |
| table | 0.04 | 0.007 | 0.06 |
| sofa | 0.02 | 0.002 | 0.04 |
| roof | 0.01 | 0.0004 | 0.03 |

$T=0.5$ 时，"mat"的概率从0.35跃升至0.54，后几个词几乎没有机会被选中——分布变得很"尖"。$T=1.5$ 时，分布明显变平，"roof"从0.01升至0.03，低概率词也获得了更多机会。

**Step 3：Top-k = 4** → 只保留 \{mat, floor, bed, chair\}，重新归一化后概率分别为 \{0.41, 0.29, 0.18, 0.12\}。从这4个词中随机采样，"table"以下的词完全没有机会。

**Step 4：Top-p = 0.9** → 按概率从高到低累积：mat(0.35) + floor(0.60) + bed(0.75) + chair(0.85) + rug(**0.93** ≥ 0.9) → Nucleus = \{mat, floor, bed, chair, rug\}，共5个词。

**Top-p的自适应性**：如果概率分布更集中——比如"United States of"之后 "America" 概率为0.92——那么 Nucleus 只包含1个词，效果等价于贪心。如果分布更分散，Nucleus 会自动扩大。这就是 top-p 优于 top-k 的关键。

| 策略 | 本例候选集 | 核心特点 | 典型应用场景 |
|------|-----------|---------|-------------|
| Greedy | 1个（mat） | 确定性，易退化 | 极短生成、严格场景 |
| Beam Search ($B$=3) | 3个候选序列 | 准确但无聊 | 机器翻译、摘要 |
| Top-k ($k$=4) | 固定4个 | 简单但不自适应 | 通用（需手动调$k$） |
| Top-p ($p$=0.9) | 动态5个 | 自适应多样性 | **LLM默认选择** |
| Temperature | 全词汇表 | 控制分布形状 | 常与top-p组合使用 |

#### 退化问题与repetition penalty

即使使用了采样方法，自回归生成仍然面临一个顽疾：**重复退化（repetition degeneration）**。模型有时会陷入循环，反复生成相同的短语或句子。这个问题在贪心解码和束搜索中尤为严重，但在采样方法中也会出现——因为一旦模型生成了某个短语，它会成为上下文的一部分，进而提高该短语再次出现的概率，形成正反馈循环。

**Repetition Penalty**（Keskar et al., 2019）是最常用的应对手段。它对已经出现过的token施加惩罚，降低其再次被选中的概率：

$$
z_i' = \begin{cases} z_i \;/\; \theta & \text{if } w_i \text{ 已出现且 } z_i > 0 \\ z_i \;\times\; \theta & \text{if } w_i \text{ 已出现且 } z_i \leq 0 \end{cases}
$$

其中 $\theta > 1$ 是惩罚系数。直觉上，对于已出现的token，如果它原本是高概率的（$z_i > 0$），就降低其logit；如果原本是低概率的（$z_i \leq 0$），就进一步压低。

OpenAI的API提供了更精细的变体。**Frequency Penalty** 根据token出现的**次数**成比例惩罚——出现越多，惩罚越重，适合抑制频繁重复。**Presence Penalty** 则是一次性的惩罚——只要出现过就惩罚，不管出现了几次，适合鼓励模型谈论新话题。两者可以组合使用。

#### 洞察：没有"最优"解码策略

一个重要的认识是：**不存在适用于所有场景的"最优"解码策略**。解码策略的选择本质上是**质量-多样性权衡（quality-diversity trade-off）**的体现。

机器翻译和文本摘要追求准确与忠实——采样引入的随机性是有害的，确定性方法（贪心或小束宽束搜索）通常更合适。创意写作和开放式对话追求多样性和"惊喜感"——确定性方法过于保守，top-p加上适当温度能生成更自然、有趣的文本。代码生成则处于两者之间——语法结构需要确定性，但算法选择允许一定的多样性。

这个权衡在后续章节中还会反复出现。第21章讨论的Chain-of-Thought推理对解码策略非常敏感——Self-Consistency方法正是通过多次高温采样生成不同的推理路径，然后用多数投票来提升推理可靠性。

到这里，我们理解了GPT如何从概率分布中生成文本。但GPT的真正突破不止于生成——它更重要的贡献是建立了"预训练+微调"的迁移范式。接下来让我们看看微调是如何工作的。
...
**Holtzman, A., Buys, J., Du, L., Forbes, M., & Choi, Y. (2020). "The Curious Case of Neural Text Degeneration"**。揭示了最大化解码（贪心、Beam Search）在开放式生成中导致文本退化的现象，并提出Nucleus（Top-p）Sampling作为解决方案。重点阅读：Section 3（退化现象的定量分析——重复率、n-gram统计）、Section 4（Nucleus Sampling的形式化定义与实验验证）。本章解码策略部分的核心参考。[arXiv:1904.09751](https://arxiv.org/abs/1904.09751)

**Fan, A., Lewis, M., & Dauphin, Y. (2018). "Hierarchical Neural Story Generation"**。在故事生成任务中首次提出Top-k Sampling方法，通过截断概率分布的长尾来避免采样到不合理的低概率词。虽然论文的主要贡献是层级式故事生成架构，但Top-k Sampling这一采样策略对后续的文本生成研究产生了深远影响。[arXiv:1805.04833](https://arxiv.org/abs/1805.04833)
